...
export default function Posts() {
  ...
  return (
    <div className="relative flex max-w-2xl flex-col rounded-lg border border-neutral-200 dark:border-neutral-800 shadow-md">
      <div className="h-[calc(3*theme(spacing.24))] overflow-y-auto px-4 pt-2 pb-6">
        ...
        {!isLoading &&
          posts?.map((post) => (
            <Link
              key={post.id}
              href={`/posts/${post.id}`}
              className="hover:bg-neutral-100 dark:hover:bg-neutral-800 flex flex-col rounded-lg px-2 pt-3 pb-4 transition-all hover:underline"
            >
